...
if(!isset($_POST['item_id']) || trim($_POST['item_id'])=='' || !isset($_POST['itemurl']) || trim($_POST['itemurl'])=='' || !isset($_POST['category_id']) || trim($_POST['category_id'])==''|| !isset($_POST['itemName']) || trim($_POST['itemName'])=='' || !isset($_POST['user_id']) || trim($_POST['user_id'])=='')
{
  ...
  $sql= "UPDATE wishlist_item 
          SET item_name = '$item_name', item_url = '$item_url', image_url = $img_url, brand = $brand, category_id= $category_id WHERE item_id=$item_id ;";
...
<div class="container row mt-5 mx-auto">
            ...
            <h2> <?php echo $item_name;?> was successfully edited!</h2>
          </div>
				<?php endif; ?>
        <div class="col-10 col-lg-5 mx-auto">
        <a href="<?php  if(isset($error) && trim($error) != '' && isset($user_id) && trim($user_id)){echo "view_results.php?user_id=". $user_id;} else if(isset($error) && trim($error) != ''){ echo "view.php";} else { echo "view_results.php?user_id=". $user_id;}  ?>" class="btn btn-dark mt-4 rounded-pill">Go back </a>
        </div>
    </div>
</div>
